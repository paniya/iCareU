...
elseif(isset($_SESSION['ID']) || $_POST['Debg']=="true"){ // see if the session is present
	...
	elseif ($func == "sendCode"){ // send the password reset code
		if(isset($_POST['email'])){
			$qry = "SELECT * FROM users WHERE ID IN (SELECT GuardianID FROM Guardian WHERE email = '".$_POST['email']."');";
			$result = mysqli_query($con,$query);
			if (mysqli_num_rows($result) == 1){
				$record = mysqli_fetch_assoc['$result'];
				$code = rand(123456,987654);
				if ($con -> query("UPDATE users SET Code = '".$code."' WHERE ID = '".$record['GuardianID']."';")){
					// send email
					echo "done";
				}
				else echo "error";
